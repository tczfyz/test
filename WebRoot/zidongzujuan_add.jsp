...
<html>
  <head>
    ...
    <title>自动组卷</title><LINK href="CSS.css" type=text/css rel=stylesheet>
...
function gow()
{
  ...
  添加自动组卷:
  <br><br>
   <table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF" style="border-collapse:collapse">    <tr><td>编号：</td><td><input name='bianhao' type='text' id='bianhao' value='' />&nbsp;*</td></tr><tr><td>专业：</td><td><select name='zhuanye' id='zhuanye'><%String sql="select zhuanye from zhuanyexinxi order by id desc";ResultSet RS_result=connDbBean.executeQuery(sql);while(RS_result.next()){%><option value="<%= RS_result.getString("zhuanye")%>" ><%=RS_result.getString("zhuanye")%></option><%}%></select></td></tr><tr><td>题数：</td><td><input name='tishu' type='text' id='tishu' value='' />&nbsp;*</td></tr><tr><td>分值：</td><td><input name='fenzhi' type='text' id='fenzhi' value='' />&nbsp;*</td></tr>
    <tr>
      <td>&nbsp;</td>
